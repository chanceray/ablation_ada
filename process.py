...
def full_load_data(dataset_name, splits_file_path=None, lbl_noise=0, seed=42):
    ...
    labels = add_label_noise(train_idx, labels, lbl_noise, seed)
...
def add_label_noise(idx_train, labels, noise_ratio, seed):
    ...
    sele_idx = [j for j in random.sample(list(idx_train), noise_num)]
    for i in sele_idx:
        re_lb = random.sample([j for j in range(max(labels)+1) if j != labels[i]], 1)
        labels[i] = re_lb[0]
    return labels